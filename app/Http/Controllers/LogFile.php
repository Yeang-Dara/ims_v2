    ...
    public function uploadFile1(Request $request)
{
    try {
        ...
        // Check if file storage was successful
        if (!$inputFilePath) {
            throw new \Exception('Failed to store uploaded file.');
        }

        // Output CSV file path
        $outputFileName = date('Y-m-d_H-i-s') . '_output.csv';
        ...
        // Check if CSV writer initialization was successful
        if (!$csvWriter) {
            throw new \Exception('Failed to initialize CSV writer.');
        }
        ...
        // Log any exceptions
        \Log::error('Error in uploadFile method: ' . $e->getMessage());

        // Optionally, return a response with an error message
        return response()->json(['error' => 'An error occurred during file upload. Please try again.'], 500);
    }
}
    ...
    public function uploadFile(Request $request)
    {
        ...
        // Store the uploaded file in the 'input' directory within the storage/app directory
        $inputFilePath = $file->storeAs('input', 'input.txt', 'local');
        Storage::disk('local')->get($inputFilePath);
        // Output CSV file path
        $outputFilePath = 'output/' . date('Y-m-d_H-i-s') . '_output.csv';
        // Use Laravel's CSV writer
        $csvWriter = Writer::createFromPath(storage_path("app/$outputFilePath"), 'w+');
